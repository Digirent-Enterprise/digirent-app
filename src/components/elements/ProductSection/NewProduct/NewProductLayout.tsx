...
const NewProduct = () => {
  ...
  useEffect(() => {
    ...
    const sortedProducts = newOne.sort((itemA, itemB) => (itemA.createdDate > itemB.createdDate) ? -1 :  1);
        ...
        {newProducts &&
          ...
          newProducts!.map(item =>
              <Link to={`/product/${item._id}`} >
                <ProductItem key={item._id} name={item.name} images={item.images} rentalCost={item.rentalCost}/>
              </Link>
              )}
           </>
        }
